...
source("./R/code_district_variable.R")
...
source("./R/standardization.R", encoding = "UTF-8")
...
source("./R/datamining_pop.R", encoding = "UTF-8")
...
## 지도화 함수
source("./R/map_regional.R", encoding = "UTF-8")


# 4.1 데이터 셋 지정 및 권역변수 코딩

data_set11_17 <- dt_dg_2011_2017 
data_set11_17 <- code_district_daegu(data_set11_17)

data_set11_14 <- dt_dg_2011_2017[dt_dg_2011_2017$josa_year %in% 2011:2014, ] 
data_set11_14 <- code_district_daegu(data_set11_14)

data_set15_17 <- dt_dg_2011_2017[dt_dg_2011_2017$josa_year %in% 2015:2017, ] 
data_set15_17 <- code_district_daegu(data_set15_17)

## 4.2 조율 및 표준화율 산출 --------------------------------

index_vector <- tb2[1, ]

stan_pop_rate <- chs_by_age_sex(data_set11_17, de_var= index_vector[[3]], var_name = index_vector[[2]])

result_c_rate <- regional_chs_analysis(data_set11_17, de_var = index_vector[[3]], denominator = index_vector[[4]][[1]], by_year = index_vector[[5]], 
                                       var_name = index_vector[[2]], stan_rate = stan_pop_rate, pop = p, target_years = 2011:2017)
...
map_f <- data_to_map(map_dong_dg, data = result_c_rate[[1]], map_merge_key = "adm_dr_nm", map_key = "adm_dr_cd", data_key = "dong")

plot_map(map_f, re_var = "c_rate", index_type = index_vector[6], legend_label = "조율(%)", color_type = "viridis") +
  ggrepel::geom_label_repel(data = region_name_bogun, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
  theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)

### 동별 시군구(중구)
map_01 <- map_f[map_f$sigungu_cd == "010", ]
dong_name_01 <- name_in_map(map_dong_dg[map_dong_dg$sigungu_cd == "010",], name_var = "adm_dr_nm")

plot_map(map_01, re_var = "c_rate", index_type = index_vector[6], legend_label = "조율(%)", color_type = "viridis") +
  ggrepel::geom_label_repel(data = dong_name_01, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
  theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)

### 권역
map_f_d <- data_to_map(map_district_dg, data = result_c_rate[[2]], map_merge_key = "district", map_key = "district", data_key = "district")

plot_map(map_f_d, re_var = "c_rate", index_type = index_vector[6], legend_label = "조율(%)", color_type = "viridis") +
  ggrepel::geom_label_repel(data = region_name_district, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
  theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)

### 보건소별(시군구)
map_f_bogun <- data_to_map(map_bogun_dg, data = result_c_rate[[3]][,-ncol(result_c_rate[[3]])], 
                           map_merge_key = "bogun_cd", map_key = "sigungu_cd", data_key = "BOGUN_CD")

plot_map(map_f_bogun, re_var = "c_rate", index_type = index_vector[6],  legend_label = "조율(%)", color_type = "viridis") +
  ggrepel::geom_label_repel(data = region_name_bogun, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
  theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)

## 5. 2011-2017 지역사회건강조사 대구시 주요지표 산출 ########################################

for (i in c(1:nrow(tb2))) {
  index_vector <- tb2[i,]
  
  stan_pop_rate <- chs_by_age_sex(data_set11_17, de_var= index_vector[[3]], var_name = index_vector[[2]])
  
  result_c_rate <- regional_chs_analysis(data_set11_14, de_var = index_vector[[3]], denominator = index_vector[[4]][[1]], by_year = index_vector[[5]], 
                                         var_name = index_vector[[2]], stan_rate = stan_pop_rate, pop = p, target_years = 2011:2017)
  ...
  map_1 <- plot_map(map_f, re_var = "c_rate", index_type = index_vector[6], legend_label = "조율(%)", color_type = "viridis") +
    ggrepel::geom_label_repel(data = region_name_bogun, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
    theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)
  
  sigungu <- c("010","020","030","040","050","060","070", "310")
  sigungu_name <- c("중구", "동구","서구","남구","북구","수성구","달서구","달성군")
  for (k in 1:length(sigungu)) {
    map_01 <- map_f[map_f$sigungu_cd == sigungu[k], ]
    dong_name_01 <- name_in_map(map_dong_dg[map_dong_dg$sigungu_cd == sigungu[k],], name_var = "adm_dr_nm")
    
    map2 <- plot_map(map_01, re_var = "c_rate", index_type = index_vector[6], legend_label = "조율(%)", color_type = "viridis") +
      ggrepel::geom_label_repel(data = dong_name_01, aes(x=clong, y=clat, group = NULL, fill = NULL, label = name), family = "NanumGothic") +
      theme(axis.title = theme.ax, plot.title = theme.ti, legend.title = theme.leti, legend.text = theme.lete)
    ggsave(map2, paste0("./result/daegu/2011-2017/보건소별/", sigungu_name[k], "/",index_vector[[2]],"_", sigungu_name[k],"_동별.png"))
  }
